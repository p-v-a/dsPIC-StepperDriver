...
void UpdatePid (PidControllerParameters *controller) {
    ...
    _Q16 x1 = kOne + _Q16div(_Q16mpy (controller->n, controller->ts), kTwo);
    _Q16 x2 = kOne - _Q16div(_Q16mpy (controller->n, controller->ts), kTwo);
    
    controller->a1 = _Q16div(kTwo, x1);
    controller->a2 = _Q16neg(_Q16div(x2, x1));

    controller->b1 = controller->kp;
    controller->b2 = _Q16neg(_Q16mpy(_Q16div(controller->kp, x1), kTwo));
    controller->b3 = _Q16div(_Q16mpy(controller->kp, x2), x1);
    
    controller->c1 = _Q16div(_Q16div(_Q16mpy(controller->ki, controller->ts), x1), kTwo);
    controller->c2 = _Q16div(_Q16div(_Q16mpy(_Q16mpy(_Q16mpy(controller->ki, controller->ts), controller->ts),_Q16ftoi(controller->n)), kTwo), x1);
    controller->c3 = _Q16neg(_Q16div(_Q16div(_Q16mpy(_Q16mpy(controller->ki, controller->ts), x2), kTwo), x1));
    controller->c4 = _Q16div(_Q16div(_Q16mpy(controller->kt, controller->ts), x1), kTwo);
    controller->c5 = _Q16div(_Q16div(_Q16mpy(_Q16mpy(_Q16mpy(controller->kt, controller->ts), controller->ts),_Q16ftoi(controller->n)), kTwo), x1);
    controller->c6 = _Q16neg(_Q16div(_Q16div(_Q16mpy(_Q16mpy(controller->kt, controller->ts), x2), kTwo), x1));

    controller->d1 = _Q16div(_Q16mpy(controller->kd, _Q16ftoi(controller->n)),x1);
    controller->d2 = _Q16neg(_Q16div(_Q16mpy(_Q16mpy(controller->kd, _Q16ftoi(controller->n)), kTwo),x1));
    controller->d3 = _Q16div(_Q16mpy(controller->kd, _Q16ftoi(controller->n)),x1);
}
...
_Q16 CalculatePIDOutput (PidControllerParameters *c, PidControllerState *p, _Q16 reference_command, _Q16 plant_output) {
    ...
    p->u0 = _Q16mac(c->c3, p->e2, p->u0);
    p->u0 = _Q16mac(c->c4, p->eus0, p->u0);
    p->u0 = _Q16mac(c->c5, p->eus1, p->u0);
    p->u0 = _Q16mac(c->c6, p->eus2, p->u0);
